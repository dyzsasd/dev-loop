...
byte-parity-guarded), plus the two operator features the multi-provider field week asked for.

- **refactor(cli): `cli-agentops` dispatcher split** — the 1.7.0 self-audit's worst row
  (ONE switch, CC 162, CRAP 374). Now one function per verb (doc/mirror/ticket sub-verbs get
  their own) behind a flat `VERBS` table; `main()` is a lookup. Bodies moved verbatim — the
  stdout byte-parity contract (sugar ≡ op dispatcher ≡ stdio, asserted by the cli-agentops
  test) is the regression net.
- **refactor(config): `validateTeamFile` split** — CC 105 at 97.5% coverage (complexity alone
  kept it on the worst list). One module-scope validator per config section (team block /
  providers / repo registry / projects / shared ownership / probes+policy), the E/W emission
  ORDER preserved (it is observable via `WsValidationError`), the section closures hoisted
  with an `Emit` param.
- **feat(hub): §8 exact-title dedupe at the write** (`save_issue` create). An exact-title
  (trim+casefold) duplicate of a NON-terminal ticket is refused **409** with the existing
  ticket named — "comment the new observation, don't refile", now enforced where the write
  happens (field origin: the loop generated byte-identical duplicate titles sweep had to
  Cancel after the fact). Terminal titles stay reusable; `allowDuplicate:true` is the
  deliberate-refile escape hatch. All transports share the one implementation.
- **feat(team): `dev-loop team set-model`** — the one-command model switch the Qwen→Gemini
  hot-swap week was missing (it took five hand edits, a sync, and a mis-killed process):
  `set-model <agent> <model> [--project k] [--effort e] [--coding-agent c] [--team-default]`
  = validated config write + automatic `opencode.json` re-sync when the model's prefix is a
  `team.providers` entry + the restart pointer (`stop && run --background`, where the startup
  preflight verifies the model resolves).
- **ci(ratchet tightened):** post-refactor rescan → the threshold drops from 380 to the new
  ceiling (see the design doc's updated table).

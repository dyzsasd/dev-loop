...
| `budget.perFireUsd` | Per-fire estimated spend ceiling in USD. A positive number terminates a single fire whose elapsed estimated cost crosses this value; the fire is recorded with `errorClass:"budget-per-fire"` distinct from a wall-timeout. Unset = **OFF** (no per-fire cap, until Child 4 of budget-ceiling derives and ships the default). Set via `dev-loop team set team.budget.perFireUsd <n>`. | ✓ `team.budget.perFireUsd` |
...
| `agents` | Per-agent coding CLI, model, effort, cadence, and timeout overrides. Fields: `codingAgent`, `model`, `effort`, `cadence`, `fireTimeout` (max wall-clock per fire; `"0"` = off; default `"1h"`), `stallTimeout` (kill if no stdout for this long; `"0"` = off; default `"10m"` for opencode, off for claude/codex). Duration strings: `"30m"`, `"1h"`, `"10s"`. Resolution order: per-agent config > explicit `--fire-timeout`/`--stall-timeout` CLI flag > per-lane/global default. Validated by `E17` — a bad value names the offending agent + field. | — |
...
- `team.mode` (`dry-run`|`live`) · `team.linearTeam` · `team.git.defaultBranch` ·
  ...
  `team.budget.perFireUsd` (positive number USD; per-fire estimated spend ceiling)

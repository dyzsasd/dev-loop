...
- **Notify reconciliation (opinionated):** **keep §9 PM one-shot AND the daemon notifier, coexisting.** §9/PM owns the **first ping** at intake on **every** backend (linear/local have no daemon), de-duped by the `notified` label; the daemon owns **only the reminders** and is **service-only**. To close the t=0 seam (a state-based park could drop §9's `blocked+needs-pm+external-prereq` trigger labels): **PM keeps that label triple set alongside the `Human-Blocked` state**, so §9 still fires t=0 and the daemon only ever sends reminders (skips a ticket until time-in-state ≥ cadence). *(This is option (a) of the t=0 seam — chosen over "daemon owns the whole lifecycle" because it keeps the first-ping cross-backend and reuses the existing `notified` dedup.)*

...
- **The sanctioned route has a verb (WS-C C6): `dev-loop system propose|list|show|resolve`.** An agent that believes a governing file (a SKILL, `conventions.md`, `dev-loop.json`) should change files a proposal into a FILE inbox — `<workspace>/.dev-loop/system-inbox/<ts>-<slug>.md`, frontmatter (`id`, `from` actor, `fireId`, `target`, `severity`, `status open|accepted|rejected|applied`, `created`) + the proposal body — instead of editing. `propose` is agent-callable (filing authorises nothing); `resolve` is operator-only and refuses under a fire marker in the approvals-cli shape (exit 4, no bypass named). The inbox is deliberately NOT a hub table: it is text the operator reads and applies by hand as a git commit, it travels with the workspace, and it works on the linear backend too. `dev-loop status` counts open proposals in the decision queue, so the firewall now has a queue an operator in any harness can see (hub/src/system-propose.ts).

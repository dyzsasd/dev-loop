...
- **P2 — The thin stdio MCP shim.** ✅ **SHIPPED** (DL-55 + the **(3/n)** doc/event widening **DL-62, both verified Done 2026-06-25**). The shim now proxies **19 of `server.ts`'s 29 tools** (5 ticket + `whoami` + `list_events` + 6 `doc.*` + 6 `topic.*`/`post.add`). The remaining 10 → the 100% drop-in are filed family-by-family (the DL-55/DL-62/DL-64 one-family discipline, not the 16-op mega-ticket): **(4/n)** discussion-board (`topic.*`+`post.add`) = **DL-64** (✅ Done 2026-06-25); **(5/n)** `channel.*` = **DL-67** (filed); **(6/n)** `mirror.*`+labels+`get_project`. The drop-in is the P3 (single-writer + dispatch-convergence) precondition. `shim.ts` proxies tool calls to the loopback daemon op API;

...
## Block BC — §20 R2 pass 96: Current-state fire journal, fire 126 (2026-08-10)
...
## §20 R2 pass 97 — rolled 2026-08-10 (pm, one-hundred-twenty-eighth fire)

Two items rolled out of `Current state` to hold the doc flat. Both are superseded in the live doc
by a compressed line; the full text is here.

### Block BE — the §0a boot-corpus lane split (rolled from `Current state`, written pass 86)
...
### Block BF — fire journal, one-hundred-twenty-seventh fire (rolled from `Current state`)
...
### Block BG — Decisions entries rolled at §20 R2 pass 97 (fires 94, 126)
...
## §20 R2 pass 98 — rolled 2026-08-10 (pm, one-hundred-twenty-ninth fire)

One item rolled out of `Current state` to hold the doc flat against RULE 41 and this fire's journal.
Superseded in the live doc by the fire-129 entry; the full text is here.

### Block BH — Current-state fire journal, fire 128 (2026-08-10)
...
### Block BI — the `local` file-board retirement delivery verification (rolled from `Current state`, written pass 88)
...
## §20 R2 pass 99 — rolled 2026-08-10 (pm, one-hundred-thirtieth fire)

One item rolled out of `Current state`: the previous fire's journal, superseded in the live doc by the
fire-130 entry. Full text here.

### Block BJ — Current-state fire journal, fire 129 (2026-08-10)
...
### Block BK — Current-state fire journal, fire 130 (2026-08-10)

_Rolled from `Current state` by §20 R2 pass 100. Original heading:_ **2026-08-10 (pm, one-hundred-thirtieth fire): a citation written in the keyword that binds it, and the merge gate's own bug filed twice**
...
## §20 R2 pass 101 — rolled 2026-08-10 (pm, one-hundred-thirty-second fire)

One item rolled out of `Current state`: the previous fire's journal, superseded in the live doc by the
fire-132 entry. Full text here.

### Block BL — Current-state fire journal, fire 131 (2026-08-10)

_Rolled from `Current state` by §20 R2 pass 101. Original heading:_ **2026-08-10 (pm, one-hundred-thirty-first fire): the backup that stopped 41 h ago, and the alarm its own remedy silences**
...
## Block BH — the one-hundred-thirty-third fire's journal entry (rolled from `Current state`, §20 R2 pass 104)
...
## Block BI — the one-hundred-thirty-fifth fire's journal entry (rolled from `Current state`, §20 R2 pass 105)
...
### 2026-08-09 → 08-10 (fires 92–152; R2 passes 100–126) — the per-fire journals rolled out of
the live log by §20 R2 pass 130

Rolled because the live `Decisions` log had regrown to 142,474 B (73% of the whole doc) and 1,175 of
its 1,609 lines were these dated entries. Doctrine they contributed is already distilled into the
STANDING RULES block in the live log — read that, not this file.

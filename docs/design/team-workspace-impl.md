...
| 文件 | 动作 | 里程碑 | 内容 |
|---|---|---|---|
| `hub/src/team-config.ts` | 新增 | M1 | 1.x workspace schema 类型 + 解析/校验(E-codes)+ 三级解析 + 兼容视图 |
| `hub/src/workspace.ts` | 新增 | M1 | workspace 发现(cwd 爬升)、状态路径 API、`workspaces.json` 索引自愈 |
| `hub/src/paths.ts` | 改造 | M1 | workspace 感知的 dataDir/hubDb;**删除** 0.x 候选链(breaking) |
| `hub/src/resolve-project.ts` | 改造 | M1 | DL-13 匹配器喂 registry;`inferProjectForRepo`;`resolveIdentity` 走 workspace config |
...
| `hub/src/run-agents.ts` | 改造 | M1(读 workspace config)/ M3(team 调度) | 兼容视图接入;WRR 轮换、enabled/weight、team 锁、`--plan` |
...
| `references/conventions.md` / `config-schema.md` | 改造 | 每里程碑 | §25 team、§19 重写、§7/§12c 路径、§9b intake、1.x workspace schema |
...
M1 要求"运行时只读 workspace config"但**对 fire 行为零变化**。做法:所有既有消费者
...
### 4.2 `dev-loop doctor`(workspace 模式)+ `dev-loop team repair`

- doctor:workspace 命中时先跑 workspace config 检查(E-codes 全量、W01–W04、repo path 存在且是 git repo、
...
§22(报告路径 R1 形状)、§16(comms env 名规则)。config-schema.md:1.x workspace schema +
...
| ID | 任务 | 主要文件 | 规模 | 依赖 |
|---|---|---|---|---|
| D1.1 | 1.x workspace schema 类型 + 解析/校验(E01–E10)+ effective*/infer/owner API | team-config.ts | L | — |
| D1.2 | workspace 发现(env>索引>爬升)+ 路径 API + 索引自愈 | workspace.ts | M | D1.1 |
| D1.3 | paths.ts workspace 感知;删 0.x 候选链(breaking) | paths.ts | S | D1.2 |
...
| D1.7 | doctor workspace 检查(只读)+ `team repair` | doctor.ts, team-repair.ts | M | D1.2 |
| D1.8 | cli.ts 路由/usage;NEEDS_NODE_SQLITE 更新 | cli.ts | S | D1.6 |
| D1.9 | conventions §25/§19 + config-schema 1.x workspace schema + CHANGELOG | references/* | M | D1.1 |
...
| D2.6 | **devplatform 实跑**(init→add/sync→sync-project 补 ID)+ 换机 runbook | M | D2.1–D2.3 |
...
| T1.4 | test/doctor-workspace.ts | E-code 呈现;doctor 前后 workspace 文件 mtime/内容不变(只读契约);team repair 修坏 gitdir 的 worktree + WAL truncate + 索引重登记 | M |
| T1.5 | test/run-agents.ts **改造** | fixture 换 workspace 形态;既有 30+ 断言全绿(兼容视图证明);新增:workspace 内 cwd 推断、共享 repo cwd → ambiguous 报错文案 | M |
| T1.6 | npm test 链 + version-sync/build-artifact/consistency 适配 | 打包含新入口;dist chmod 断言沿用 | S |
...
| ID | 覆盖 |
|---|---|
| T6.1 | 第二台机器换机演练:§10.3 清单逐步执行 + `doctor` 全绿 + 一次真实 fire(记录 runbook) |
...
| 里程碑 | 发布判据 |
|---|---|
| M1/0.30.0 | T1.1–T1.6 全绿;`npm test` 全绿;两 team workspace dry-run 计划人工核对通过 |

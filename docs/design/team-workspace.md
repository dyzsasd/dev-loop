...
- **daemon 生命周期随 workspace(#17)**:hub 数据在 workspace 内,启停也必须以 workspace
  ...
  pid/port,幂等)/ `dev-loop hub stop`(优雅停 + WAL checkpoint,**迁移前必跑**)/
...
| 里程碑 | 版本 | 内容 | 验收 |
|---|---|---|---|
| **M1 配置内核** | 0.30.0 | schema v2(registry + 虚拟 project)+ 解析器(含共享 repo 的 project 推断/报错)+ 状态入 workspace + `team init` / `team import` / doctor。**breaking:运行时不再读 v1**(断裂发生在 0.x 线内,合规) | 解析器/上限校验/推断歧义全测试面;两个 team 的 import 干跑 diff |
...
| **M3 team 调度** | 0.32.0 | team 级唯一 loop:轮换 + weight + enabled;Agent View 行数按 agent 种类;`--project` 降级为过滤器 | 双 project 轮换烟测;`enabled:false` 全停验证 |
| **M4 stewardship + 文档库** | 0.33.0 | sweep/ops/reflect/communication 升 team 作用域;**lessons 库(§5.1:INDEX/分片/archive + 装载预算 + 可选镜像)**;comms(slack/lark)适配器 | ops 去重巡检 + owner 路由用例;INDEX 触顶下沉用例;lark 真实推送 |

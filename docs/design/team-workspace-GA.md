...
> (rc.1 → rc.3 → 1.0.0). The 0.x config path was removed at 1.0.0 as a clean break;
> the soak gate was waived by operator decision. Full suite:
...
|---|---|---|
| **M1** config kernel | `team-config.ts` (1.x workspace schema, E01–E11, resolution API, `toLegacyView`), `workspace.ts` (discovery + `.dev-loop/` paths + self-healing index), `team init/repair`, doctor workspace checks, config wiring (server/daemon/run all read the workspace config) | team-config (56), workspace (25), team-cli (27) |
...
lifecycle, agent-api, shim, migrate, …) is byte-identical green — the 0.x fixed-project path and the daemon
...
- **I4 portability** — all run state (incl. the service `hub.db`) lives under `<workspace>/.dev-loop/`;
  `~/.dev-loop` holds only a rebuildable index. `team repair` fixes
...
- **I3 one-team-one-backend** — one backend per workspace; no cross-team collaboration.
- **toLegacyView de-risk** — every existing consumer reads the workspace config through the compat view; the fire behavior
...
2. **backoffice service workspace** (D5.5): `dev-loop team init --backend service` on the
   real backoffice; verify the service hub and web board.
...
## Workspace quickstart (devplatform, linear)
...
/dev-loop:add-project       # (in a coding CLI) creates/syncs the backend project
/dev-loop:add-repo          # registers the repo and records build/deploy facts

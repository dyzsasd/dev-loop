...
Usage (from inside the workspace):
  dev-loop team repair [--dry-run]

Does: git worktree repair + prune for every registered repo, re-register the workspace index,
(service backend) checkpoint+truncate the hub WAL, and reap worktrees whose ticket is in a
terminal state (Done / Canceled / Duplicate).
...
export async function teamRepair(argv = process.argv.slice(2)): Promise<number> {
  ...
  // 4. Reap worktrees whose ticket is in a terminal state (Done / Canceled / Duplicate). LOOP-37.
  //    Enumerated from `git worktree list` so all roots are covered, incl. legacy / /tmp paths.
  for (const ref of Object.keys(ws.file.repos)) {
    const dir = effectiveRepo(ws, ref).absPath;
    ...
    if (!dryRun && result.reaped.length > 0) pass(`repo '${ref}': reaped ${result.reaped.length} terminal worktree(s)`);
  }

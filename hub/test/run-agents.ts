...
try {
  ...
  ok(/mcp_servers\.dev-loop-hub\.command="[^"]*node[^"]*"/.test(codex.out), "codex dry-run DEFINES the hub server via -c with an absolute, version-checked node bin (PATH-independent under launchd/systemd)");

...
const OPERATOR_SKILLS = ["add-project", "add-repo", "operator-console", "playbooks", "sync-project", "sync-repo"]; // legacy `init` removed at 1.0; operator-console added by one-click §3; `playbooks` = the shared job-playbook library (job-scoped prompts), a non-agent fragment dir the boot assembler pulls, never launched

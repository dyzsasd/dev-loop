...
try {
  ...
  ok(JSON.stringify(stdioTools) === JSON.stringify(shimTools) && shimTools.length === 27, `the shim proxies ALL ${stdioTools.length} server.ts tools — a 100% drop-in (got shim=${shimTools.length}, stdio=${stdioTools.length})`);
